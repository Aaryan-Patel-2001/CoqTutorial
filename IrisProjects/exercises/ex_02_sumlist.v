...
Lemma inc_list_spec_induction n l v :
  {{{ is_list l v }}}
    inc_list #n v
  {{{ RET #(); is_list (map (Z.add n) l) v }}}. (* Understadning specs: we are adding number n to each element of list n, and ensure that the value is equal to v?    *)
Proof.
  (* exercise *)
  iIntros (Φ) "Hl Post".
  iInduction l as [|x l] "IH" forall (v Φ); simpl. 
  ...
  - iDestruct "Hl" as %->.
    ...
    iModIntro. (*Introduce a modality in the proof goal *)
    iApply "Post". (* Match the conclusions of current goal aganist conclusions of "Post" *) 
    iPureIntro. (* Turn a pure goal, of the form phi, into a coq goal *)
    reflexivity.  
  - iDestruct "Hl" as (p) "[-> Hl]". iDestruct "Hl" as (v) "[Hp Hl]".
    wp_rec. (* Unfold def *)
    wp_let. (* Reduce a let binding*)
    wp_match. (* Reduce a match construct*)
    wp_load. (*Reduce a load operation: Find the points to assertion*) 
    wp_proj. (*Reduce a projection *)
    wp_let.
    wp_load. wp_proj. wp_let.
    wp_op. (* Reduce an arithematic operator*) 
    wp_store. (* Store the current value: Finds the points to assertion in the spatial contexts *) 
    wp_apply ("IH" with "Hl"). (* Applying te induction hypothesis for inc_list *) 
    iIntros "Hl".
    iApply "Post". 
    iExists p. (* Wittness for ∃ *) iSplitR; [done|] (*  split a conjunction P ∗ Q into two proofs.*).
    iExists v. iSplitR "Hl"; [iApply "Hp"|iApply "Hl"].
Qed. 
...
Proof.
  ...
  iIntros (Φ) "Hl Post".
  iLöb as "IH" forall (l v Φ). destruct l as [|x l]; simpl; wp_rec. 
  - iDestruct "Hl" as %->. wp_pures. iModIntro. iApply "Post". iPureIntro. reflexivity.  
  - iDestruct "Hl" as (p -> v) "[Hp Hl]". wp_let. wp_match.  
