...
Proof.
  (* exercise *)
  iIntros (Φ) "(Hx & Hy & Hz) Post".
  unfold rotate_l. wp_lam. do 2 wp_let.
  wp_bind (swap _ _).
  iApply (swap_spec with "[Hx Hy]").
  { iFrame. }
  iNext. iIntros "[Hx Hy] /=". wp_seq.
  iApply (swap_spec with "[Hy Hz]").
  { iFrame. }
  iNext. iIntros "[Hy Hz]". iApply "Post". iFrame.  
Qed. 
End proof.
...
resource: https://github.com/izgzhen/iris-coq/blob/master/ProofMode.md

iIntros: Introduce universal quantifiers Φ and implication/magic wands using (Hx & Hy & Hz) Post

unfold: Apply the definition 

wp_lam: weakest precondition 
